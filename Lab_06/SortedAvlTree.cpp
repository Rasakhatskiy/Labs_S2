...
SortedAvlTree::Node::Node() :
        Left(nullptr), Right(nullptr), Height(0), Value(std::string())
{}

SortedAvlTree::Node::Node(std::string value) :
        Left(nullptr), Right(nullptr), Height(0), Value(std::move(value))
{}
...
int SortedAvlTree::GetSize() const
{
    return Size;
}
...
{
	if (!targetNode->Right) return targetNode;
...
SortedAvlTree::Node *SortedAvlTree::Insert(std::string value, Node *targetNode)
{
    ...
    if (balance > 1 && targetNode->Left && value.size() < targetNode->Left->Value.size())
    {
        return RotateRight(targetNode);
    }

    if (balance < -1 && targetNode->Right && value.size() > targetNode->Right->Value.size())
    {
...
SortedAvlTree::Node *SortedAvlTree::Remove(std::string value, Node *targetNode)
{
    ...
    } else
    {
        ...
        if (balance > 1 && targetNode->Left && Balance(targetNode->Left) >= 0)
        {
            return RotateRight(targetNode);
        } else if (balance > 1 && targetNode->Left && Balance(targetNode->Left) < 0)
        {
            targetNode->Left = RotateLeft(targetNode->Left);
            return RotateRight(targetNode);
        } else if (balance < -1 && targetNode->Right && Balance(targetNode->Right) <= 0)
        {
            return RotateLeft(targetNode);
        } else if (balance < -1 && targetNode->Right && Balance(targetNode->Right) > 0)
        {
